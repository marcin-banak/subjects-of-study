...
(define (extend-env [env : Env] [x : Symbol] [exp : Exp]) : Env ; Przypisuje Exp do zmiennej zamiast value
  (cons (bind x exp) env))
    ...
    [(ifE b l r)
     (type-case Value (eval b env)
       ...
       [else
        (error 'eval "type error")])]
    [(varE x)
     (eval (lookup-env x env) env)] ; wyliczamy wartość wyrażenia przypisanego do zmiennej
    [(letE x e1 e2)
       (eval e2 (extend-env env x e1))] ; rozszrzamy środowisko o Exp
    [(lamE x b)
     (funV x b env)]
    [(appE e1 e2)
     (apply (eval e1 env) e2)])) ; przekazujemy do apply całe wyrażenie zamiast jego wartości

(define (apply [v : Value] [e : Exp]) : Value ; apply rozszerza środowisko o Exp
  (type-case Value v
    [(funV x b env)
