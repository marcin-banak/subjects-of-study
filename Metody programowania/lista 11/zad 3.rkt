...
(define-type Exp
  ...
  (varE [x : Symbol])
  (letE [bs : (Listof (Symbol * Exp))] [e : Exp])
  (let*E [x : Symbol] [e1 : Exp] [e2 : Exp]))
...
(define (let*-desugaring [vs : (Listof (Symbol * Exp))] [e : Exp])
  (if (empty? (rest vs))
      (let*E (fst (first vs)) (snd (first vs)) e)
      (let*E (fst (first vs)) (snd (first vs)) (let*-desugaring (rest vs) e))))
...
(module+ test
  ...
  (test (parse `{let* ((x 1)) {+ x 1}})
        (let*E 'x (numE 1) (opE (add) (varE 'x) (numE 1)))))
...
(define (extend-env-list [env : Env] [bs : (Listof (Symbol * Value))]) : Env
  (if (empty? bs) env
      (cons (bind (fst (first bs)) (snd (first bs))) (extend-env-list env (rest bs)))))
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(letE bs e)
       (eval e (extend-env-list env (map (lambda (x) (pair (fst x) (eval (snd x) env))) bs)))]))
