...
(define (extend-env-list [env : Env] [xs : (Listof Symbol)] [vs : (Listof Value)]) : Env
  (if (empty? xs) env
      (cons (bind (first xs) (first vs)) (extend-env-list env (rest xs) (rest vs)))))
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(lamE xs b)
     (funV (λ (v) (eval b (extend-env-list env xs v))))]
    [(appE f es)
     (apply (eval f env) (map (lambda (e) (eval e env)) es))]))

(define (apply [v1 : Value] [vs : (Listof Value)]) : Value
  (type-case Value v1
    [(funV f) (f vs)]
